...
def timerEvent():
    ...
    find_text(root)[0].text = 'BLA'
    find_text(root)[0].set('transform', "rotate({},3,1)".format(angle))
    23.811/2

    find_text1(root)[0].set('transform', "rotate({},12.1,16.1)".format(angle,int(svgWidget.width()/2),int(svgWidget.height()/2)))
...
find_text1 = etree.ETXPath("//{http://www.w3.org/2000/svg}path[@id='path15']")
